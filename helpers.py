...
def add_suffix_to_filename(filename: str, suffix: str) -> str:
    ...
    """
    # If suffix starts with a dash, don't add an underscore
    connector = '' if suffix.startswith('-') else '_'

    if filename.endswith('.nii'):
        result = filename.replace('.nii', f'{connector}{suffix}.nii')
        return result
    elif filename.endswith('.nii.gz'):
        result = filename.replace('.nii.gz', f'{connector}{suffix}.nii.gz')
        return result
    else:
